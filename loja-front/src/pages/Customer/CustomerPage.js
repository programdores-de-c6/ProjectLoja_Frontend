...
import ShopForm, { ShopSchema as ShopDataSchema } from "./CustomerForm.js";
...
import { useCustomerService as SupllierService } from "./CustomerService.js";
 ...
 const fetchlocation = async () => {
    try {
      ...
      const adaptados = response.map((l) => ({
        ...
        contactosecudario: l.contectoSecudario || l.contectoSecudario,
        localidade: l.idLocations
          ? { value: l.idLocations, label: l.nomeLocations }
          : null,
        localidadeNome: l.nomeLocations || "",
  ...
  const handleEdit = (supplier) => {
    const dadosParaModal = {
      ...supplier,
      localidade: supplier.localidade
        ? { id: supplier.localidade.value, nome: supplier.localidade.label }
        : null,
  ...
  const handleNew = () => {
    setData({ nome: "", contribuinte: "", contactoprincipal:"", contactosecudario:"", email:""}); 
...
// 🔹 Abrir modal de visualização
  ...
  caixaPostal: shopData.caixaPostal || "—",
    ...
    setViewShop({ fields, image: supplier.logo, entity, title: "Detalhes do Cliente" });
    } catch (err) {
  ...
  return (
    <PageContainer>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 gap-2">
        <h3 className="text-primary fw-bold m-0">Cliente</h3>
        <input
          type="text"
