...
const criar = async (data) => {
  try {
    ...
    const supplierDto = {
      ...
      country: data.pais
        ? { id: data.pais.id, nome: data.pais.nome }
        : null,
    };

   
    const res = await axios.post(`${api}/supplier/create`, supplierDto, {
      headers: {
 ...
 const editar = async (data) => {
  try {
   ...
   const res = await axios.put(`${api}/supplier/update`, payload, {
       headers: {
          Authorization: `Bearer ${user?.accessToken}`, // token do contexto
        },
        withCredentials: true, // se houver refresh token
 ...
 return { listar, criar, editar, listarpais: listarPais, deletar };
};